...
outidfile=$tmppath/outsinkid.tmp
if [ -f $idfile ]; then oldid=$(< $idfile); fi
if [ -f $outidfile ]; then oldoutid=$(< $outidfile); fi
...
start () {
	...
	outidnum=$(pactl load-module module-null-sink sink_name=$viperoutsink sink_properties=device.description="Viper4Linux-out")
	echo $idnum > "$idfile"
	echo $outidnum > "$outidfile"
	...
	! pulsesink device="$viperoutsink" > $logfile &
	echo $! > $pidfile
